"""
Cross-modal attention + gating fusion for CDSA-ATM — NumPy reference
implementation (v2).

Reference: Vaswani et al. (2017), Attention Is All You Need, NeurIPS.

Token set: {h_trajectory, h_ats, h_coord}. Per-modality sigmoid gates,
scaled dot-product attention, gated sum, linear head -> fusion_dim for
the Multi-Agent PPO Actor-Critic head. Gating scores are exposed for
the XAI layer and the platform FRL Agent Panel.

Scope note (Faz A): compact CPU-scale forward implementation.
Framework-scale training under TUBITAK 1001 ARDEB Project 4.
"""
...
class CrossModalAttentionFusion:
    """Gated cross-modal attention over the three ATM modality latents."""

    MODALITIES = ("trajectory", "ats", "coord")
    ...
    def forward(self, h_trajectory: np.ndarray, h_ats: np.ndarray,
                h_coord: np.ndarray, return_gates: bool = False):
        H = np.stack([np.asarray(h, dtype=float)
                      for h in (h_trajectory, h_ats, h_coord)])
